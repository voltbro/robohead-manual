...
2. Проиграет аудиофайл **`smile.mp3`** через динамики.
...
2. Скопируйте в папку два медиафайла:
...
1. **Инициализация ROS-ноды**  
   Мы вызываем `rospy.init_node("smile_node")`, чтобы зарегистрировать новый узел в ROS. Переменная `script_path` содержит полный путь к папке со скриптом, чтобы затем корректно формировать пути к медиафайлам.
