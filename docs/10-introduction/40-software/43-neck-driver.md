...
```
Если введённое значение выходит из диапазона 0..4095, оно будет округлено до ближайшей границы.
...
7. Возьмите **общие** границы. Обычно выбирают более “узкий” диапазон. Пример: у серво 1 получился диапазон `112...560`, у серво 2 — `107...547`. Тогда возьмите диапазон `112..547`. Рекомендуется уменьшить найденный диапазон на 10-20 пунктов с каждого конца для устойчивой работы: в примере получим `130...530`.
...
- Это обычно следствие перепутанных каналов `servo_1_channel` и `servo_2_channel`. Поменяйте либо физическое подключение к плате, либо каналы в конфиг-файле.
