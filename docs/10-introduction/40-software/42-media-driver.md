...
> **Важно:** Стриминг не влияет на аудиоплеер и не требует остановки текущего воспроизведения (оно останавливается автоматически)

#### Пример
Запустите пакет `usb_cam`: он получает данные с камеры и публикует их в топик `/image_raw`.
```bash
...
`s0`, `s1`,... — номера слотов (один слот — один палец). `id56`, `id57` — уникальный номер касания: остаётся одним и тем же, пока палец не оторван от экрана, при повторном касании инкрементируется на единицу.
