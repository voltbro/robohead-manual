...
Основные функции, доступные пользователям из "коробки":
...
Все пользовательские сценарии разрабатываются через **ROS-пакеты** и/или специальеую обёртку на языке Python, что обеспечивает простую интеграцию с аппаратной частью. Например, можно:
...
Через пакет `turtlebro_voice_nav` Робоголова передаёт голосовые команды для управления движением TurtleBro2.
