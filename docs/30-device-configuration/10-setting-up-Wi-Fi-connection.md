...
sidebar_position: 1
description: "Настройка подключения к Wi-Fi"
---

При включении устройства у вас есть два способа подключения к Робоголове: **сеть Wi-Fi** или **проводное соединение**.
...
### Настройка подключения к новой сети Wi-Fi через SD-карту
...
Для добавления новой сети Wi-Fi необходимо отредактировать файл `/etc/netplan/20-network-wifi.yaml`. Для этого:
...
3. Внутри файла `/etc/netplan/20-network-wifi.yaml` будет содержимое
   ...
   ```
4. В этом файле в поле `wlan0` добавьте свою сеть Wi-Fi.

Пример файла с новой сетью Wi-Fi:
...
3. При необходимости вернитесь к разделу **Настройка подключения к новой сети Wi-Fi** и отредактируйте конфигурацию.
